...
export function MobileMoodCalendar({ entries }: MobileMoodCalendarProps) {
  ...
  const getDayColor = (day: number) => {
    ...
    if (dayEntries.length === 0) return '#F5F8FA';
      ...
      <div className="max-w-md mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl" style={{ color: '#2D7A8B' }}>
              Mood History
            </h2>
          </div>
          <div 
        ...
        <div className="bg-white rounded-3xl p-6 mb-6 shadow-sm">
          ...
          <div className="flex items-center justify-between mb-6">
            <button onClick={handlePrevMonth} className="p-2">
              <ChevronLeft className="w-5 h-5" style={{ color: '#2D7A8B' }} />
            </button>
            <h3 className="text-lg" style={{ color: '#2D7A8B' }}>
              {monthNames[month]} {year}
            </h3>
            <button onClick={handleNextMonth} className="p-2">
              <ChevronRight className="w-5 h-5" style={{ color: '#2D7A8B' }} />
            </button>
          </div>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                >
                  <span style={{ color: hasEntries ? 'white' : '#A8C9C7' }}>
                    {day}
                  </span>
                  {dayEntries.length > 1 && (
                    <div 
        ...
        {/* Legend */}
        <div className="bg-white rounded-3xl p-6 mb-6 shadow-sm">
          <h3 className="text-sm mb-4" style={{ color: '#2D7A8B' }}>
            Mood Legend
          </h3>
          <div className="space-y-3">
            {Object.entries(MOOD_COLORS).map(([mood, color]) => (
              <div key={mood} className="flex items-center gap-3">
                ...
                <span className="text-sm capitalize" style={{ color: '#2D7A8B' }}>
                  {mood.replace('-', ' ')}
                </span>
              </div>
            ))}
        ...
        {selectedEntry && (
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div>
                <h3 className="text-lg mb-1" style={{ color: '#2D7A8B' }}>
        ...
        {entries.length === 0 && (
          <div className="bg-white rounded-3xl p-12 text-center shadow-sm">
            <div 
              ...
              style={{ backgroundColor: '#E8F6F8' }}
            >
              <CalendarIcon className="w-10 h-10" style={{ color: '#A8C9C7' }} />
            </div>
            <h3 className="text-lg mb-2" style={{ color: '#2D7A8B' }}>
              No Mood History Yet
            </h3>
